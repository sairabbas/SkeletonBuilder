...
void Joint::draw() {
	...
	if (parent)
	{
		...
		glm::vec3 dist = parent->getPosition() - getPosition();
		float len = glm::length(dist);
		glm::vec3 norm = glm::normalize(dist);
		glm::mat4 rot = rotateToVector(glm::vec3(0, 1, 0), norm);
		glm::mat4 trans = glm::translate(getPosition());
		glm::mat4 offset = glm::translate(glm::vec3(0, len / 2, 0));
		bone.height = len - 2 * radius;
		//Draw bone
		ofPushMatrix();
		ofMultMatrix(trans * rot * offset);
		bone.draw();
		ofPopMatrix();
	}
}
bool Joint::intersect(const Ray &ray, glm::vec3 &point, glm::vec3 &normal) {
	//Convert ray coordinates to object space
	glm::mat4 inverse = glm::inverse(getMatrix());
	glm::vec4 inverseP1 = inverse * glm::vec4(ray.p.x, ray.p.y, ray.p.z, 1.0);
	glm::vec4 inverseP2 = inverse * glm::vec4(ray.p + ray.d, 1.0);
	glm::vec3 dist = glm::normalize(inverseP2 - inverseP1);
	return (glm::intersectRaySphere(glm::vec3(inverseP1), dist, glm::vec3(0, 0, 0), radius, point, normal));
}
